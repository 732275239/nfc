package com.chuyu.nfc.tools.bar;
...
import com.chuyu.nfc.tools.OSUtils;
...
/**
 * Created by Zoello on 2019/1/22.
 */
...
//        if (fontIconDark) {
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
                ...
                setTranslucentStatus(activity);//透明
                if (fontIconDark) {
...
//                setStatusBarColor(activity,Color.LTGRAY);//灰色
     ...
     * 设置文字颜色
     */
    public static void setStatusBarFontIconDark(Activity activity,@ViewType int type,boolean abc) {
        switch (type) {
            case TYPE_MIUI:
                setMiuiUI(activity,abc);
                break;
            case TYPE_M:
                setCommonUI(activity);
                break;
            case TYPE_FLYME:
                setFlymeUI(activity,abc);
                break;
        }
    }
...
//        StatusbarColorUtils.setStatusBarDarkIcon(activity.getWindow(),dark);
        try {
            Window window = activity.getWindow();
